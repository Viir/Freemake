...
updateAnimateCameraSingleStep progressAmountMilli consoleBefore =
  ...
  then consoleBefore
    ...
    let
      ...
      applyFadeOut = not (distanceToPan <= cameraTransitionPanningDistanceThreshold)

      (cameraVelocityUpdatedForTransition, fadeOut) =
        if applyFadeOut
        then (consoleBefore.cameraVelocity, consoleBefore.cameraFadeOut + progressSeconds)
        else
      ...
      (cameraVelocity, cameraOffset) =
        if 1 <= fadeOut
        then (Vector2d.zero, cameraDestinationOffset)
        else
          let
            velocity = cameraVelocityUpdatedForTransition |> Vector2d.scaleBy dampFactor
          in
